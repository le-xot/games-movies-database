...
- `ast_grep` pattern `$Enums.Foo.$VAR` only matches _value_ usages, NOT type annotations like `$Enums.Foo` used as a TypeScript type — scan remaining files manually after ast_grep runs
