...
- [x] 1. Fix all 30 lint warnings — config + code
  ...
  **C. Убрать пустой конструктор** (1 файл): 4. `backend/src/modules/auth/auth.apikey.guard.ts:14` — удалить строку `constructor() {}` (и пустую строку после неё). Класс имеет field initializer `private readonly logger = new Logger(...)`, конструктор не нужен.

  **D. Исправить variable shadowing** (5 файлов, 7 warnings): 5. `frontend/src/composables/factories/create-records-store.ts` — 2 исправления:
